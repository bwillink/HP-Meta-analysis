...
resource_sim_summary = data.frame("resource_r" = r_sim_inputs, "ht" = ht_sim_inputs, "S_eq" = S_result, "I_eq" = I_result, "R_eq" = R_result,
...
The left-most line is when $\beta$ is most sensitive to resources and the right-most is least sensitive.
...
to the environment (βE)") +
                    ...
                    geom_line() #+ geom_text(aes(label=Beta_sensitivity))
environment_plot


```

The top line is when $\beta$ is most positively affected by the environmental factor and the bottom line is for most negatively affected.
### Simulations with C
If you are a Windows user, you can run the simulations with C using the following code
```{r}
rtools <- "C:\\Rtools\\bin"
