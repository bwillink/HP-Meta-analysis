...
# this one includes one just barely negative eigen value
#dat_1 <- dat_1[-grep("Waki", dat_1$Study),]
...
val <- eigen(varcovmat_1)
val$values[which(val$values < 0)]
isSymmetric(varcovmat_1)

# exclude studies with negative eigen values that make the matrix not positive definite
dat_2 <- dat_2[-grep("Ashraf", dat_2$Study),]

# these ones include just barely negative eigen value
#dat_2 <- dat_2[-grep("Waki", dat_2$Study),]
...
val <- eigen(varcovmat_2)
val$values[which(val$values < 0)]
isSymmetric(varcovmat_2)
...
isSymmetric(varcovmat_2_byHost)
```
