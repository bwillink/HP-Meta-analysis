...
accepted_var <- c("SE", "SD", "CI", "Wald CI", "IQ", "OR", "t", "Z", "Z_reg", "F_X")
...
Based on these eigenvalues, we tentatively remove the studies/experiments that cause the most trouble. Some we know have issues with reporting N, other we need to go back to and double check
```{r var-covar-matrix}
# exclude studies with negative eigenvalues that make the matrix not positive definite
dat_1 <- dat_1[-grep("Ashraf et al. 2017 Environ Sci Pollut Res", dat_1$Study),]
dat_1 <- dat_1[-grep("Pochini and Hoverman. 2017. Environmental Pollution", dat_1$Study),]
dat_1 <- dat_1[-grep("Schluter-Vorberg et al. 2017. Aquatic Toxicology", dat_1$Study),]
dat_1 <- dat_1[-grep("Shostak et al. 2015. Journal of Parasitology", dat_1$Study),]
dat_1 <- dat_1[-grep("Waki & Yoshinaga 2015. FishPathology", dat_1$Study),]
dat_1 <- dat_1[-grep("Civitello et al. 2020. Proc B", dat_1$Study),]
dat_1 <- dat_1[-grep("Waki&Yoshinaga. 2013. Fish Sci", dat_1$Study),]

# Civitello et al. 2020 has relatively high covariances for uninfected demographic effects of resource limitation because the variance in the control is extremely low. this results in negative eigenvalues.For now, I'm removing the experiment.
...
# exclude/double check studies with negative eigenvalues that make the matrix not positive definite
dat_2 <- dat_2[-grep("Ashraf et al. 2017 Environ Sci Pollut Res", dat_2$Study),]
dat_2 <- dat_2[-grep("Schluter-Vorberg et al. 2017. Aquatic Toxicology", dat_2$Study),]
dat_2 <- dat_2[-grep("Waki&Yoshinaga. 2013. Fish Sci", dat_2$Study),]
dat_2 <- dat_2[-grep("Souto et al. 2015. VetMicrobiol.", dat_2$Study),]
dat_2 <- dat_2[-grep("Purcell et al. 2016 J Fish Diseases", dat_2$Study),] #Variance is low but everything checks out for Purcell et al 2016
dat_2 <- dat_2[-grep("Bradley et al. 2019. Plos One", dat_2$Study),]
