...
Q1m3 <-
  rma.mv(
    abs(g) ~ Trait.category : Trait.type : Gradient.category -1,
    V = varcovmat_1_PD$mat,
    random = list( ~ 1 |
                     as.factor(ID), ~ 1 | as.factor(Experiment), ~ 1 | Parasite),
    data = dat_1,
...
## effects in  prevalence vs intensity measures of resistance under different types of stress
```{r Q2-single-fixed}
Q2m4 <-
  rma.mv(
    abs(g) ~ Trait.type*Gradient.category ,
...
### Stress type  x Trait type (prevalence or intensity) interaction
```{r Q2-Trait-x-type}
Q2m5 <-
  rma.mv(
    abs(g) ~ Gradient.category:Trait.type -1,
    V = varcovmat_2_PD$mat,
    random = list( ~ 1 |
                     ID, ~ 1 | Experiment, ~ 1 | Parasite),
    data = dat_2,
...
## Question 3 - Do resistance and tolerance traits have different sensitivity to stressors
### Null model
```{r Q2-null}
Q2m1 <-
  rma.mv(
