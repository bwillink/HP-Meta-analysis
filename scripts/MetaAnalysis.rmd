...
# Fit all models
all.models <- glmulti(g ~ Trait.type*Gradient.category, 
               data=dat_2,
               V = varcovmat_2_PD$mat, 
               random = list( ~ 1 | ID, ~ 1 | Experiment, ~ 1 | Parasite),
...
# Top models
top.models <- weightable(all.models)
top.models <- top.models[top.models$aicc <= min(top.models$aicc) + 2,]
top.models

# Variable importance plot across all the models
plot(all.models, type="s")
...
mmi <- as.data.frame(coef(all.models))
...
for (j in 1:all.models@nbmods) {
   model <- all.models@objects[[j]]
