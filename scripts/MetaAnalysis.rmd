...
for (i in 1:N) {
  ...
  if (dat$Variation.Type[i] == "OR" &
    ...
    # if at leas one category is 0, we apply Yate's correction to avoid dividing by 0
    if (dat$Variation.Type[i] == "OR" &
        (dat$Success_C[i] == 0 |
        dat$Success_X[i] == 0 |
        (dat$N_C[i] - dat$Success_C[i]) == 0 |
...
This code produces a variance-covariance of sampling errors matrix, n x n, with n = number of effect sizes.  It requires the dataset (dat), Hedge's g (g), and var(g) (var.g) to be defined above. It uses "Experiment", "Level_C", "Trait", "N_C", and "N_X" columns.
```{r}
...
for(i in 1:dim(dat)[1]){
  for(j in 1:dim(dat)[1]){
    if(i == j){varcovmat[i,j] = dat$var.g[i]}else{
      if(dat[i, "Experiment"] == dat[j, "Experiment"] & dat[i, "Level_C"] == dat[j, "Level_C"] & dat[i, "Trait"] == dat[j, "Trait"]){
