...
First, we find out which studies have negative eigen values in their variance-covar matrix. This means covariance between treatment effects with the same control is larger than variance and it's an indicator of suspiciously low variance and potentially an incorrectly reported N or an error while digitizing data.
...
# Civitello et al 2020 has relatively high covariances for uninfected demographic effects of resource limitation because the variance in the control is extremely low. this results in negative eigen values.For now, I'm removing the experiment.
...
# exclude/double check studies with negative eigen values that make the matrix not positive definite
...
#dat_2 <- dat_2[-grep("Purcell", dat_2$Study),] Variance is low but everything checks out for Purcell et al 2016
dat_2 <- dat_2[-grep("Bradley", dat_2$Study),]
dat_2 <- dat_2[-grep("Doublet", dat_2$Study),]
#dat_2 <- dat_2[-grep("Naggar and Paxton. 2020. Microbial Biotechnology", dat_2$Study),] # variance may be low because Z score considers cage effect (30 bees x cage in 3 cages x treatment) but I don't know what to report as sample size. For now, I have dialed it back from 90 to 30 and this makes all eigen values positive.
